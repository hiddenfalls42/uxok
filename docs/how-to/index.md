...
- [Work with capabilities](how-to-work-with-capabilities.md) — Declare `provides` and `requires`, resolve providers with `get_capability()`, and handle conflicts and missing capabilities.
...
- [Use tags for provider selection](how-to-use-tags-for-provider-selection.md) — Filter capability providers by tag at lookup time when multiple plugins provide the same capability.
