...
- [Boot a plugin graph in order](how-to-boot-a-plugin-graph-in-order.md) — Boot a batch of sources with `core.load_plugins()` (atomic) or `core.try_load_plugins()` (best-effort, with a skip report), or fall back to manual ordering, `build_host()` factoring, and polling for capabilities that arrive asynchronously.
