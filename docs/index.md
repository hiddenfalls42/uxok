...
uxok is an async-first hot-loading plugin microkernel for Python. It provides exactly six primitives — an event bus, a hook system, a plugin registry, a capability system, a base `Plugin` class, and a tick clock — and nothing else. Everything a host application does is implemented as plugins that declare what they provide and what they require; the kernel wires them together without any plugin knowing about another directly.
...
The tick clock gives every plugin a shared counter — `core.tick` — and lets any event or hook be deferred to a precise future boundary with `emit(..., at_tick=...)`. See [The tick system](explanation/tick-system.md). *It is worth mentioning that python is python, so "precise" is a relative term.*
